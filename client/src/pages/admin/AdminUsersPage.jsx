...
const statusLabels = { active: 'Đang hoạt động', restricted: 'Bị hạn chế', suspended: 'Tạm khóa', banned: 'Đã khóa' };
...
export default function AdminUsersPage() {
  ...
  const load = () => {
    setLoading(true);
    adminApi.users({ page, limit: 20, status: status || undefined, q: query || undefined })
  ...
  return (
    <div>
      ...
      <div className="panel-heading">
        <div><h2>Người dùng</h2><p>Tìm kiếm, cảnh báo, hạn chế hoặc khóa tài khoản.</p></div>
        <form className="inline-form" onSubmit={submitSearch}>
          ...
          <Button size="sm" type="submit">Tìm</Button>
        </form>
      </div>
      <div className="filter-tabs">
        {[['', 'Tất cả'], ['active', 'Hoạt động'], ['restricted', 'Hạn chế'], ['suspended', 'Tạm khóa'], ['banned', 'Đã khóa']].map(([value, label]) => (
          <button type="button" key={value} className={status === value ? 'is-active' : ''} onClick={() => { setStatus(value); setPage(1); }}>{label}</button>
        ))}
      </div>
      {loading ? <LoadingBlock /> : items.length ? (
        <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Người dùng</th><th>Liên hệ</th><th>Xác thực</th><th>Trạng thái</th><th>Ngày tạo</th><th /></tr></thead><tbody>
