  ...
  if (tab) {
    ...
    try {
      ...
      tab.document.body.innerHTML =
        `<div style="font-family:Arial,sans-serif;padding:32px;color:#10231A">${message}</div>`;
    } catch {
      // Browser có thể chặn thao tác với about:blank; tab vẫn dùng được.
    }
  }
...
function sendTabTo(preparedTab, path) {
...
export default function ArticleEditorPage() {
    ...
    const hydrate = async () => {
      ...
      try {
        if (!routeId) {
          ...
          const next = local?.form
            ? { ...emptyForm(), ...local.form }
            : emptyForm();
          if (local?.form) {
            setNotice('Đã khôi phục bản nháp gần nhất trên máy này.');
          }
          serverIdRef.current = '';
        ...
        const useLocal =
          local?.form && Number(local.savedAt || 0) > serverTime;
        const next = useLocal
          ? { ...serverForm, ...local.form }
          : serverForm;
        if (useLocal) {
          setNotice(
            'Đã khôi phục thay đổi bạn đang soạn dở trước khi đóng trang.',
          );
        }
  ...
  const payloadOf = useCallback(
    (current, requestedStatus) => ({
      ...
      tagIds: Array.isArray(current.tagIds)
        ? current.tagIds.map(idOf).filter(Boolean)
        : [],
      thumbnailMediaId: idOf(current.thumbnailMediaId),
      status: requestedStatus || current.status || 'draft',
      scheduledAt:
        (requestedStatus || current.status) === 'scheduled'
  ...
  const saveToServer = useCallback(
    async ({ status, manual = false } = {}) => {
      const current = formRef.current;
      if (
        String(current.title || '').trim().length < 5 ||
        !hasContent(current.bodyHtml)
      ) {
        if (manual) {
          setSaveState({
            kind: 'error',
            time: '',
            message:
              'Hãy nhập tiêu đề tối thiểu 5 ký tự và nội dung bài viết.',
          });
        }
        return null;
      }
      ...
      try {
        ...
        const next = {
          ...
          thumbnailMediaId:
            item?.thumbnailMediaId || current.thumbnailMediaId,
        };
        ...
        if (savedId) {
          ...
          if (!routeId) {
            navigate(`/quan-tri/bai-viet/${savedId}/sua`, {
              replace: true,
            });
          }
        }
  ...
  useEffect(() => {
    if (
      loading ||
      !revision ||
      formRef.current.status === 'published'
    ) {
      return undefined;
    }
    const timer = window.setTimeout(
      () =>
        saveToServer({
          status: formRef.current.status || 'draft',
        }),
      1800,
    );
  ...
  useEffect(() => {
    const preserve = () =>
      writeDraft(serverIdRef.current || 'new', formRef.current);
    const onVisibility = () => {
  ...
  const closeEditor = () => {
    ...
    if (window.opener || window.history.length <= 1) {
      window.close();
      window.setTimeout(() => {
        if (!window.closed) navigate('/quan-tri/bai-viet');
      }, 80);
      return;
    }

    navigate('/quan-tri/bai-viet');
  };

  const openDocs = async () => {
    const preparedTab = prepareTab('Đang mở Google Docs…');
    ...
    sendTabTo(preparedTab, `/quan-tri/bai-viet/${id}/docs`);
  };
  ...
  const syncDocs = async () => {
    ...
    setSaveState({
      kind: 'saving',
      time: '',
      message: 'Đang đồng bộ tiêu đề, mô tả và nội dung Google Docs…',
    });
    ...
    try {
      ...
      setSaveState({
        ...
        time,
        message:
          'Đã đồng bộ tiêu đề, mô tả và nội dung Google Docs về website.',
      });
      toast.success(
        'Đã đồng bộ tiêu đề, mô tả và nội dung mới nhất từ Google Docs.',
      );
    } catch (error) {
      const message = apiErrorMessage(error);
      setSaveState({ kind: 'error', time: '', message });
  ...
  const openPublicArticle = () => {
    const slug = String(formRef.current.slug || '').trim();
    if (!slug || formRef.current.status !== 'published') return;

    const tab = window.open(
      new URL(`/tin-tuc/${slug}`, window.location.origin).toString(),
      '_blank',
    );
    if (tab) tab.opener = null;
  };
  ...
  const googleSyncTitle = form.googleDocSyncedAt
    ? `Đồng bộ lần cuối: ${syncDateLabel(form.googleDocSyncedAt)}`
    : 'Lấy tiêu đề, mô tả và nội dung mới nhất từ Google Docs';

  if (loading) {
    return (
      <main className="admin-doc-launcher">
        <section className="admin-doc-launcher-card">
          <div className="admin-doc-launcher-orb">ĐT</div>
  ...
  return (
    <main className="admin-article-studio">
      <Seo
        title={serverIdRef.current ? 'Chỉnh sửa bài viết' : 'Bài viết mới'}
      />
      <header className="admin-article-studio-bar">
        <button
          type="button"
          className="admin-article-studio-back"
          onClick={closeEditor}
        >
          <X size={15} /> Đóng phòng soạn
        </button>
        <div className={`admin-article-save-state ${saveState.kind}`}>
          <span />
          {saveLabel(saveState)}
        </div>
        <div className="admin-article-studio-actions">
          {form.googleDocId ? (
            <button
              ...
              onClick={syncDocs}
              title={googleSyncTitle}
            >
              <RefreshCw size={14} /> Đồng bộ Docs
            </button>
          ) : null}
          <button
            type="button"
            ...
            onClick={openDocs}
          >
            <Cloud size={14} /> Google Docs ↗
          </button>
          {form.status === 'published' && form.slug ? (
            <button
              type="button"
              className="admin-secondary"
              onClick={openPublicArticle}
            >
          ...
          <button
            ...
            disabled={saveState.kind === 'saving'}
            onClick={() =>
              saveToServer({ status: 'draft', manual: true })
            }
          >
          ...
          <button
            ...
            disabled={saveState.kind === 'saving'}
            onClick={() =>
              saveToServer({ status: 'published', manual: true })
            }
          >
            <Send size={14} />
            {form.status === 'published' ? 'Cập nhật' : 'Xuất bản'}
          </button>
        </div>
      </header>
      ...
      <div className="admin-article-studio-canvas">
        {notice ? (
          <div className="admin-article-recovery">↻ {notice}</div>
        ) : null}
        {saveState.message ? (
          <div
            className={`admin-alert ${
              saveState.kind === 'error' ? 'error' : 'success'
            }`}
          >
            {saveState.message}
          </div>
        ...
        <section className="admin-article-basics">
          <div className="admin-article-title-wrap">
            <p className="admin-kicker">
              {serverIdRef.current ? 'Chỉnh sửa bài viết' : 'Bài viết mới'}
            </p>
            <textarea
              rows={2}
              value={form.title}
          ...
          <label className="admin-article-excerpt">
            ...
            <textarea
              rows={3}
              value={form.summary}
              onChange={(event) => apply('summary', event.target.value)}
              placeholder="Tóm tắt ngắn giúp người đọc và công cụ tìm kiếm hiểu bài viết…"
            />
        ...
        <section className="admin-article-writing-area">
          <div className="admin-article-writing-heading">
            <div>
              <p className="admin-kicker">Nội dung chi tiết</p>
              <h2>Không gian soạn bài TipTap</h2>
            </div>
            <p>
              Dán nội dung từ Word/Docs, Ctrl + V hoặc kéo thả ảnh, tạo bảng,
              căn lề, heading, liên kết và danh sách ngay trong bài.
            </p>
          </div>
          <ArticleComposer
            value={form.bodyHtml}
            onChange={(value) => apply('bodyHtml', value)}
        ...
        <section className="admin-article-meta-panel">
          ...
          <div className="wide">
            <MediaUploader
              ...
              onChange={(value) => apply('thumbnailMediaId', value)}
            />
          </div>
          <label className="wide">
            <span>Nguồn và ghi chú biên tập</span>
            <textarea
              rows={4}
