  ...
  return (
    <article className="community-card">
      <header>
        ...
        <div>
          <Link to={author.username ? `/thanh-vien/${author.username}` : '#'}>{author.displayName || 'Thành viên'}</Link>
          <div><span>{formatRelativeTime(item.publishedAt || item.createdAt)}</span><VerifiedMark emailVerifiedAt={author.emailVerifiedAt} phoneVerifiedAt={author.phoneVerifiedAt} compact /></div>
        </div>
        <Badge tone="soft">{COMMUNITY_TYPES[item.community?.postType] || 'Cộng đồng'}</Badge>
      </header>
      <h3><Link to={`/cong-dong/${item.slug}`}>{item.title}</Link></h3>
      {item.summary ? <p>{truncate(item.summary, 220)}</p> : null}
      {item.thumbnailMediaId ? <Link to={`/cong-dong/${item.slug}`}><ContentImage media={item.thumbnailMediaId} alt={item.title} /></Link> : null}
      <footer>
