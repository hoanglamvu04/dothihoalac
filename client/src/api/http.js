...
const refreshClient = axios.create({ baseURL, withCredentials: true, timeout: 20000 });
  ...
  async (error) => {
    ...
    if (status === 401 && !config.__retried && !isAuthEndpoint) {
      ...
      try {
        ...
        // AuthContext sẽ đưa giao diện về trạng thái chưa đăng nhập.
      }
    }
...
export function apiErrorMessage(error, fallback = 'Có lỗi xảy ra. Vui lòng thử lại.') {
...
export function unwrapList(response) {
  return {
    ...
    meta: response?.data?.meta || { page: 1, limit: 12, total: 0, totalPages: 1 },
  };
}
