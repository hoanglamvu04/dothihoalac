...
export async function myActivity(req, res) {
  const r = await service.listMyActivity(req.user._id, req.query);
  return sendSuccess(res, {
    data: r.items,
    meta: {
      ...r.meta,
...
export async function clearMySearchActivity(req, res) {
  return sendSuccess(res, {
    data: await service.clearMySearchActivity(req.user._id),
