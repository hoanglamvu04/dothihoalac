...
import {
  adoptKthlGoogleConnection,
  getKthlSharedConnectionStatus,
} from './googleWorkspace.shared.js';
...
function cleanClientOrigin(value) {
  try {
    return new URL(value || env.CLIENT_URL).origin;
  } catch {
...
function oauthRedirect(clientOrigin, params = {}) {
  const url = new URL(
    ...
    cleanClientOrigin(clientOrigin),
  );
...
function activeGoogleRedirectUri() {
  ...
  try {
    ...
    const activePort = Number(
      process.env.DTHL_ACTIVE_PORT || 0,
    );
...
function assertGoogleOAuthRuntimeConfig() {
  ...
  return {
    ...
    allowedDomain: String(
      env.GOOGLE_WORKSPACE_ALLOWED_DOMAIN || '',
    )
      .trim()
      .toLowerCase(),
  };
...
function createRuntimeGoogleAuthorizationUrl(
  state,
) {
  const config =
...
async function exchangeRuntimeGoogleAuthorizationCode(
  code,
) {
  const config =
    assertGoogleOAuthRuntimeConfig();
...
async function ensureArticleDocument(contentId) {
  ...
  if (!file) {
    file =
      await findGoogleDriveDocumentByArticle(
        accessToken,
        String(content._id),
      );
  }
  ...
  if (!file) {
    ...
    file = await createArticleGoogleDoc(
      accessToken,
      {
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      clientOrigin =
        state.clientOrigin || clientOrigin;

      if (req.query.error) {
        throw new GoogleWorkspaceError(
          `Google từ chối kết nối: ${req.query.error}`,
          'GOOGLE_OAUTH_DENIED',
          400,
        );
      }
      ...
      const tokenData =
        await exchangeRuntimeGoogleAuthorizationCode(
          req.query.code,
        );
      ...
      const drive = await getGoogleDriveAbout(
        tokenData.access_token,
      );
      ...
      if (!connection.refreshTokenEncrypted) {
        throw new GoogleWorkspaceError(
          'Google không trả về refresh token. Hãy kết nối lại và chấp thuận quyền truy cập.',
      ...
      connection.sharedSourceDb = '';

      await connection.save();

      return res.redirect(
        oauthRedirect(clientOrigin, {
          google: 'connected',
        }),
      );
    } catch (error) {
      const message = String(
        error?.message ||
          'Không thể kết nối Google Workspace.',
      ).slice(0, 220);
...
router.get(
  ...
  asyncHandler(async (_req, res) => {
    const [config, connection, sharedKthl] =
      await Promise.all([
        Promise.resolve(
          getGoogleWorkspaceConfigStatus(),
        ),
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const connection =
      await adoptKthlGoogleConnection(
        req.user._id,
      );
    ...
    return sendSuccess(res, {
      ...
      message:
        'Đã dùng lại kết nối Google Workspace của Kiến Trúc Hòa Lạc.',
    });
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const state = createGoogleOAuthState(
      req.user._id,
      req.get('origin') || env.CLIENT_URL,
    );

    return sendSuccess(res, {
      data: {
        url: createRuntimeGoogleAuthorizationUrl(
          state,
        ),
      },
    });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    return sendSuccess(res, {
      ...
      message:
        'Đã thiết lập cấu trúc Google Drive.',
    });
  }),
);
...
router.post(
  ...
  asyncHandler(async (_req, res) => {
    ...
    return sendSuccess(res, {
      ...
      message:
        'Đã ngắt kết nối Google Workspace.',
    });
  }),
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const generatedTitle =
      `Bản nháp Google Docs ${new Date().toLocaleString('vi-VN')}`;
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { connection, accessToken } =
      await loadConnectedGoogle();
    const document =
      await getGoogleDocsDocument(
        accessToken,
        article.googleDocId,
      );
    const parsed =
      parseGoogleDocsArticle(document);
    ...
    return sendSuccess(res, {
      data: await adminArticleDetail(
        content._id,
      ),
      message:
        'Đã đồng bộ nội dung từ Google Docs về website.',
    });
  }),
);
